...
public class CastGenerator {
    ...
    public void start(){
        ...
        MediaFormat mediaFormat = MediaFormat.createVideoFormat(MediaFormat.MIMETYPE_VIDEO_AVC, Common.RESOLUTION_OPTIONS[0][mIndexSize],Common.RESOLUTION_OPTIONS[1][mIndexSize]);
        mediaFormat.setInteger(MediaFormat.KEY_BIT_RATE, Common.BITRATE_OPTIONS[mIndexBitrate]);
        mediaFormat.setInteger(MediaFormat.KEY_FRAME_RATE, Common.FPS_OPTIONS[mIndexFps]);
        ...
        mediaFormat.setLong(MediaFormat.KEY_REPEAT_PREVIOUS_FRAME_AFTER,  (Common.DEFAULT_I_FRAME_INTERVAL*2000000)/Common.FPS_OPTIONS[mIndexFps]);
        // ------------------------------------------------------------------------------------------------
...
//                        Log.e(TAG,"录屏 .."+b.length+" == is key "+isIFrame(frame));
                        // 添加数据到队列
                        getAppInfo().getScreenStream().add(b);
                        if(getAppInfo().getConnectionManager() != null && getAppInfo().getConnectionManager().isConnect()){
                            getAppInfo().getConnectionManager().send(new CastRequest(b));
                        }
                  }
                    if (mMediaCodec != null) {
